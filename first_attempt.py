...
KNOWN_TRAP_PATTERNS = [
    ...
    r'.*/~eppstein/pix/.*',
    r'.*redirect_to=.*', 
    r'.*wics\.ics\.uci\.edu.*share=.*',
    r'.*wics\.ics\.uci\.edu.*r\.php.*',
    r'.*ngs\.ics\.uci\.edu.*wp-login\.php.*'
]
...
def is_valid_domain(domain):
    ...
    for allowed in allowed_domains:
        if domain == allowed or domain.endswith('.' + allowed): # special handling cecs.uci.edu
            return True
...
def is_infinite_trap_url(url):
    """
    prevent falling into infinite traps
    """
    parsed = urlparse(url)
    ...
    # WICS.ICS.UCI.EDU TRAPS - IMPORTANT!
    if 'wics.ics.uci.edu' in domain:
        if parsed.query and 'share=' in parsed.query.lower():
            return True
        
        if 'r.php' in path_lower:
            return True
        trap_params = ['share=', 'next=', 'type=', 'link=', 'app_id=']
        if parsed.query:
            query_lower = parsed.query.lower()
    ...
    if query:
        # Block specific trap patterns
        trap_params = [
            'share=', 'redirect=', 'next=', 'page=',
            'replytocom=', 'p=', 'id=', 'session=',
            'do=', 'action=', 'view=', 'mode='
        ]
...
def is_valid(url):
    ...
    try:
        ...
        if parsed.query:
            ...
            if param_count > 8:  # Limit to 8 parameters
                return False
            
            # Block common infinite trap parameters
            trap_params = ['share=', 'next=', 'redirect_to=', 'replytocom=', 'page_id=']
    ...
    if resp.status == 200 and resp.raw_response and resp.raw_response.content:
        try:           
            ...
            if is_low_information_page(text, html_content):
                return []  # CORRECT - stop on low-info pages
            else:
